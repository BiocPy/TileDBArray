...
from tiledbarray import TileDbArraySeed
import os
import delayedarray
...
def test_TileDbArraySeed_dense():
    ...
    roundtrip = TileDbArraySeed(dir, name="")
...
def test_TileDbArraySeed_sparse():
    ...
    roundtrip = TileDbArraySeed(dir, name="")
    assert roundtrip.shape == (4, 4)

    assert delayedarray.is_sparse(roundtrip) == True

    slices = (slice(0, 2), [2, 3])
    print(delayedarray.extract_sparse_array(roundtrip, (*slices,)))
